...
router.get('/pending', async (_req, res) => {
    ...
    `);
        ...
        console.error('GET /api/travel-orders/pending error:', error.message);
...
router.get('/for-approval', async (_req, res) => {
    ...
    `);
        ...
        console.error('GET /api/travel-orders/for-approval error:', error.message);
        res.status(500).json({ success: false, data: null, error: 'Database error' });
    }
});
// GET /api/travel-orders/for-request — Fetch FOR_REQUEST orders
router.get('/for-request', async (_req, res) => {
    try {
        const pool = getPool();
        const result = await pool.query(`
      SELECT
    ...
    `);
        ...
        console.error('GET /api/travel-orders/for-request error:', error.message);
        res.status(500).json({ success: false, data: null, error: 'Database error' });
    }
});
...
router.get('/scheduled', async (_req, res) => {
    ...
    `);
        ...
        console.error('GET /api/travel-orders/scheduled error:', error.message);
        res.status(500).json({ success: false, data: null, error: 'Database error' });
    }
});
...
router.patch('/:id/assign', async (req, res) => {
    ...
    try {
        ...
        const vehicleResult = await pool.query('SELECT id, plate_number FROM vehicles WHERE id = $1', [vehicle_id]);
        ...
        const driverResult = await pool.query('SELECT id FROM drivers WHERE id = $1', [driver_id]);
        ...
        const updateResult = await pool.query(`UPDATE travel_orders
       SET vehicle_id = $1, driver_id = $2, status = 'FOR_APPROVAL', updated_at = NOW()
       WHERE id = $3
       RETURNING *`, [vehicle_id, driver_id, req.params.id]);
    ...
    `, [req.params.id]);
        ...
        console.error('PATCH /api/travel-orders/:id/assign error:', error.message);
        res.status(500).json({ success: false, data: null, error: 'Database error' });
    }
});
...
router.post('/', async (req, res) => {
    const { vehicleId, driverId, originLocation, destinationLocation, scheduledDepartureAt, scheduledArrivalAt, purpose, notes, department, travelerName, requestVehicle, requestDriver, toNumber, } = req.body;
...
router.delete('/:id', async (req, res) => {
    try {
        const pool = getPool();
        const result = await pool.query('DELETE FROM travel_orders WHERE id = $1 RETURNING id', [req.params.id]);
        if (result.rows.length === 0) {
            res.status(404).json({ success: false, data: null, error: 'Travel order not found' });
            return;
        }
        ...
        console.error('DELETE /api/travel-orders/:id error:', error.message);
        res.status(500).json({ success: false, data: null, error: 'Database error' });
    }
});
